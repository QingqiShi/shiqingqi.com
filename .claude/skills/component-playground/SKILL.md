...
- A toggle line — `layer.texture`, `.wash`, `.floating`, `.scrollMask`, `.glass` — asks for a system effect, not a property. Build it the way `DESIGN.md` builds that effect. For `.glass` at 100% and the default tokens, that is `glassSurface.base` spread into the layer's `css` array alongside the layer's `corner.*` preset or its own radius. A reduced opacity scales that token's alpha in `packages/ui/src/tokens.stylex.ts`, because the glass tokens exist for Glass alone. A non-default colour token becomes a declaration on the layer after `glassSurface.base`. A radius other than 8px is a `backdropFilter` declaration on the layer after `glassSurface.base`: a step from 4px to 32px is `blur(<N>px)`; `off` is `backdropFilter: "none"`, cancelling the blur `glassSurface.base` carries — `indicatorNoBlur` in `segmented-control.tsx` is the shipped example, for a Glass over an opaque fill with nothing to sample. A layer that does not compose `glassSurface.base` simply gets no `backdropFilter` declaration for `off`.
