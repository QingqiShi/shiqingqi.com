...
// Rounded corners — radius paired with shape. Every corner in the system is a
// squircle: the radius token sizes it, this primitive supplies the shape, so
// a consumer composing one gets both without a global corner-shape rule.
...
export const corner = stylex.create({
  ...
  radius_round: { borderRadius: border.radius_round, cornerShape: "squircle" },
});
