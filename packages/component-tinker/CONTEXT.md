...
The `@tuja/component-tinker` package, and the `component-tinker` skill that drives it: together they build one throwaway HTML page where the user retunes one `@tuja/ui` component by eye and exports the change back to the agent. Design System words (Token, Primitive, Intent) keep their meaning here; the words below are this context's own.
