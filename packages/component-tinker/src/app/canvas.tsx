...
function Cell({
  ...
  useLayoutEffect(() => {
    ...
    const find = (id: string) =>
      body.querySelector<HTMLElement>(`[data-tinker-id="${id}"]`);
  ...
  const idAt = (target: EventTarget | null) =>
    target instanceof Element
      ? (target.closest("[data-tinker-id]")?.getAttribute("data-tinker-id") ??
        null)
      : null;
  ...
  return (
    <figure className={width ? "tk-cell" : "tk-cell tk-cell--full"}>
      <figcaption className="tk-cell-title">{cell.title}</figcaption>
      <div
        className="tk-cell-frame"
        style={{
          colorScheme: scheme,
      ...
      >
        <div
          className={
            hasFloating ? "tk-cell-body tk-cell-body--floating" : "tk-cell-body"
          }
          ref={bodyRef}
        ...
        >
          {hasFloating ? (
            <p className="tk-sample" aria-hidden>
          ...
          {measured.hover ? (
            <div
              className="tk-outline tk-outline--hover"
              aria-hidden
          ...
          {measured.selection ? (
            <div
              className="tk-outline tk-outline--on"
              aria-hidden
              style={boxStyle(measured.selection)}
            >
              <span className="tk-outline-tag" style={{ zoom: 1 / zoom }}>
                {selectedNode?.layer}
              </span>
            </div>
          ) : null}
...
export function Canvas({
  ...
  return (
    <div className="tk-canvas-scroll" ref={scrollRef}>
      <div className="tk-cells" style={{ zoom }}>
        {cells.map((cell) => (
          <Cell
            key={cell.index}
            cell={cell}
