...
export function Swatch({ token }: { token: TokenEntry }) {
  ...
  return (
    <span className="tk-swatch" aria-hidden>
...
function Mark({ kind }: { kind: MarkKind | null }) {
  ...
  return (
    <span className={kind === "keyword" ? "tk-keyword" : "tk-off-system"}>
      {kind}
    </span>
  );
}
...
export function ValueChip({ property, value, index }: ValueChipProps) {
  if (value === undefined) {
    return <span className="tk-value tk-value--unset">unset</span>;
  }
  ...
  if (parsed.kind === "token") {
    ...
    return (
      <span className="tk-value">
        ...
        <span className="tk-token-name">{parsed.token}</span>
        {token?.hint ? <span className="tk-hint">{token.hint}</span> : null}
      </span>
    );
  }
  return (
    <span className="tk-value">
      <span className="tk-token-name">{String(value)}</span>
...
export function PickerBody({
  ...
  return (
    <div className="tk-picker">
      {showsLiteral ? (
        <p className="tk-picker-literal">
          <span className="tk-token-name">{String(current)}</span>
          <Mark kind={markFor(parsed, property)} />
        </p>
      ) : null}
      <input
      ...
      <ul className="tk-options">
        {shownKeywords.map((keyword) => (
          <li key={keyword}>
            ...
            >
              <span className="tk-token-name">{keyword}</span>
            </button>
          </li>
        ))}
        {tokens.map((token) => (
          <li key={token.name}>
            ...
            >
              <Swatch token={token} />
              <span className="tk-token-name">{token.name}</span>
              <span className="tk-hint">{token.hint}</span>
            </button>
          </li>
        ))}
        {tokens.length + shownKeywords.length === 0 ? (
          <li className="tk-empty">Nothing matches “{query}”.</li>
        ) : null}
      </ul>
    </div>
  );
