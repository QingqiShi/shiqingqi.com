...
export function TopBar({
  ...
  return (
    <header className="tk-bar">
      <div className="tk-bar-name">
        <span className="tk-component">{component}</span>
        <span className="tk-source">{source}</span>
      </div>

      <div className="tk-bar-controls">
        <Segmented
          label="Theme"
          options={THEMES}
          value={scheme}
        ...
        {isPhone ? null : (
          <>
            ...
            />
            <button type="button" className="tk-button" onClick={onReset}>
              Reset all
            </button>
          </>
        )}
