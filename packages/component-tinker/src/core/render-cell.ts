...
import type { CellConfig, EffectStyle, TinkerConfig } from "./types.ts";
 ...
 * so every function type in a tinker tree is one of those.
 */
...
function renderNode(
  ...
  if (typeof type !== "string") {
    throw new Error(
      `A tinker tree takes host elements and plain function components only, not ${String(type)}.`,
    );
  }
