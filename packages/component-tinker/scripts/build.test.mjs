...
const EXAMPLES = [
  { file: "segmented-control.tinker.tsx", component: "SegmentedControl" },
  { file: "switch.tinker.tsx", component: "Switch" },
];
  ...
  it(
    ...
    async () => {
      ...
      const outputPaths = await Promise.all(
        EXAMPLES.map(async ({ file }) => {
          const tempDir = fs.mkdtempSync(
            path.join(os.tmpdir(), "component-tinker-build-"),
          );
          ...
          return copyPath.replace(/\.tinker\.tsx$/, ".html");
        }),
      );
      ...
      outputPaths.forEach((outputPath, index) => {
        ...
        expect(
          html.startsWith(
            `<meta charset="utf-8">\n<title>${EXAMPLES[index].component} tinker</title>`,
          ),
        ).toBe(true);
