...
import { tinkerViteConfig } from "./tinker-vite-config.mjs";
...
async function loadAndValidate(configPath, cataloguePath, tempDir) {
  ...
  await esbuild.build({
    ...
    jsx: "automatic",
    logLevel: "silent",
    alias: { "@tuja/component-tinker": corePath },
...
async function main() {
  ...
  if (!input) {
    console.error(
      "Usage: pnpm --filter @tuja/component-tinker tinker <config.tinker.tsx>",
    );
    process.exit(1);
  }
  ...
  const tempDir = fs.mkdtempSync(path.join(os.tmpdir(), "component-tinker-"));
  try {
    ...
    await build({
      ...tinkerViteConfig({ configPath, cataloguePath, outDir: distDir }),
      logLevel: "warn",
    });
...
<title>${escapeHtml(loaded.component)} tinker</title>
<style>
${adapter.css}
${appCss}</style>
<div id="tinker-root"></div>
