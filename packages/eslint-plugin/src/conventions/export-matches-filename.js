 ...
 * every export in the file is a type export, the file is a type-only module
 * and must be named `types.ts`; its type names are not checked against the
 * stem. A bare `export {}` names nothing and is not an export.
 *
...
const TYPE_ONLY_MODULE_STEM = "types";
...
const exportMatchesFilename = {
  meta: {
    ...
    messages: {
      ...
      typeOnlyModule:
        'Name a type-only module "types.ts": "{{basename}}" exports only types ({{exports}}).',
    },
    schema: [],
  },
  ...
  create(context) {
    return {
      "Program:exit"(program) {
        ...
        if (!hasValueExportStatement) {
          if (typeNames.length === 0 || wanted === TYPE_ONLY_MODULE_STEM)
            return;
          context.report({
            node: exportStatements[0],
            messageId: "typeOnlyModule",
            data: { basename, exports: typeNames.join(", ") },
          });
          return;
        }

        if (valueNames.some((name) => normalise(name) === wanted)) return;

        let exportsText =
          valueNames.length > 0 ? valueNames.join(", ") : "none named";
