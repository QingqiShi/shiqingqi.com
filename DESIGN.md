...
- **Nothing casts a shadow.** There is no light above the page, so nothing is separated by one. Glass catches light on its top rim, and that rim separates nothing.
- **Glass is the one surface that catches light, and the one that casts a shadow.** It is a lens floating above what it sits on, lit from straight above — a page cannot follow the device, so the light never tilts. Drawn the way glass is drawn: a translucent body blurring what lies beneath it, its face flat; a hairline rim in the border colour all the way round, lit on top and again along the bottom where light leaves the glass, the light gone down the sides — against a light page the rim reads as a dark edge, against a dark one it is clear and only the light on it shows; a one-pixel band inside the bottom edge, that light bounced back through the glass; and the Button's shadow beneath it. That is the stated exception to "Nothing casts a shadow" above and to a Wash's "no bright spot" below — without it, glass reads as a tint or an emboss rather than as glass. The blur is the element's own background, which is why it is not a Progressive blur: a Floating element's blur stays on the page.
