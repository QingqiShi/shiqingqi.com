...
/**
 * Build the WebGL2 pathtracer pipeline for `canvas` and start its render
 * loop, returning the controls that drive it. `undefined` when the browser
 * gives us no WebGL2 context.
 */
export function createPathtracerLoop(
  canvas: HTMLCanvasElement,
  shaders: {
    vertexShaderSrc: string;
    pathtracerFragSrc: string;
  ...
  initialDebug: DebugOptions,
  onStats?: (stats: FrameStats) => void,
): LoopControls | undefined {
  const context = createPipeline(
    canvas,
    shaders.vertexShaderSrc,
    shaders.pathtracerFragSrc,
    shaders.displayFragSrc,
  );
  if (!context) return;
  return startLoop(context, { isDark }, initialDebug, onStats);
}
