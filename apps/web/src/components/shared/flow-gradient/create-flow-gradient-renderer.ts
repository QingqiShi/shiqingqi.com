...
/** Foreground hues for the gradient, as normalised RGB triples. */
...
/**
 * Build the WebGL pipeline for `canvas`. The returned handle exposes the
 * canvas plus `start`, which begins the render loop and returns its cleanup.
 * `undefined` when the browser gives us no WebGL context.
 */
export function createFlowGradientRenderer(
  canvas: HTMLCanvasElement,
  vertexShaderSrc: string,
  fragmentShaderSrc: string,
) {
  const context = createPipeline(canvas, vertexShaderSrc, fragmentShaderSrc);
  if (!context) return;
  return {
    ...
    ) => startLoop(context, colorsRef, options),
  };
}
