/**
 * The counterpart to `../routes/`: those modules own the structure of the
 ...
 *
 * Every export here is called from a server component or from
 * `generateMetadata`, which is render scope the `no-t-outside-render` rule
 * cannot follow across a module boundary.
 */
 ...
 * `DesignSystemPath`, so a route added to `../routes/design-system-routes.ts`
 * without a name here fails to compile.
 */
