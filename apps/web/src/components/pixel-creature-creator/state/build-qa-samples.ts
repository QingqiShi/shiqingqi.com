 ...
 * Element, and accessory appears at least once across the grid before any
...
export function buildQaSamples(): QaSample[] {
  ...
  const elementOrder = shuffle(rng, ELEMENT_IDS);
  ...
  for (let i = 0; i < COMBINATORIAL_QA_COUNT; i += 1) {
    ...
    const elementId = elementOrder[i % elementOrder.length];
