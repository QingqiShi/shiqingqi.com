...
const zodFilePath = path.join(__dirname, '../src/_generated/tmdb-zod-schemas.ts');
...
content = content.replace(/zod\.array\(\)\.optional\(\)/g, 'zod.array(zod.unknown()).optional()');
content = content.replace(/zod\.array\(\)(?!\.)/g, 'zod.array(zod.unknown())');

// Count fixed arrays
const fixedArrayMatches = content.match(/zod\.array\(zod\.unknown\(\)\)(?:\.optional\(\))?/g);
const fixedCount = fixedArrayMatches ? fixedArrayMatches.length : 0;

fs.writeFileSync(zodFilePath, content);

console.log(`✅ Fixed ${brokenCount} broken Zod array schemas (converted to zod.array(zod.unknown()))`);

if (brokenCount > 0 && fixedCount === brokenCount) {
  console.log('🎉 All broken array schemas have been fixed!');
} else if (brokenCount > 0) {
  console.log(`⚠️  Expected to fix ${brokenCount} arrays, but actually fixed ${fixedCount}`);
} else {
