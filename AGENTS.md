...
- Run `pnpm dev` from the one app you work on (e.g. `apps/web`), never from the root, which starts every app. The port is per worktree — `node scripts/worktree-port.mjs` prints it; never assume 3000.
