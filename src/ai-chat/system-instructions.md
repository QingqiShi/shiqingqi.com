...
- **Watch providers**: When users ask where to watch something, use `tmdb_search` to find the TMDB ID first, then call `watch_providers`. After receiving results, call the appropriate presentation tool to display them visually — your text should complement, not repeat, the card:
